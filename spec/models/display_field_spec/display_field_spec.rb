...
describe DisplayField do
  ...
  it "should increment the display order" do
    survey = create :survey
    df1 = DisplayFieldText.create! :name => "df1", :display_order => 1,
      :survey_version => survey.survey_versions.first
    df2 = DisplayFieldText.create! :name => "df2", :display_order => 2,
      :survey_version => survey.survey_versions.first
  ...
  context "move_display_field" do
    it "should reorder display fields" do
      ...
      df1.move_display_field(2)

      expect(df2.reload.display_order).to eq(1)
    end
  end
