...
This fork is intended for Rails 3.0. If you need best_int_place in a Rails 3.1 project, maybe you should take a look to [the original project](https://github.com/bernat/best_in_place)
...
Finally, just add a binding to prepare all best in place fields when the document is ready:
...
- v.0.1.5 **Attention: this release is not backwards compatible**. Changing params from list to option hash, helper's refactoring,
  ...
  of key ESCAPE for d[M`:$estroying changes before they are made permanent (in inputs and textarea).
