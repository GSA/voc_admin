...
class CustomView < ActiveRecord::Base
  ...
  # ordered_display_fields determines how the selected display fields
  # are presented in the Custom View form's multiselect box
  #
  # @return [Array<DisplayField>] Array containing joined DisplayFields in display order
  def ordered_display_fields
    self.display_fields #.order(:display_order) <-- removed. redundant with :through ordering, right?
  end

  # sorted_display_field_custom_views determines which display fields are selected
  # on the Custom View form (and their sort orders)
  #
