...
class DisplayField < ActiveRecord::Base
  ...
  def populate_default_values!
    select_sql = <<-SQL
      ...
      where survey_responses.id not in (
        select distinct(survey_response_id)
        from display_field_values
        where display_field_values.display_field_id = #{self.id})
      and survey_version_id = #{self.survey_version.id};
    SQL

    ret = SurveyResponse.connection.execute(select_sql)

    ret.each_slice(1000) do |responses|
      attribs = []
      responses.each do |sr_id|
        attribs.push "(#{id}, #{sr_id.first}, '#{default_value}', now(), now())"
      end
      insert_sql = <<-SQL
        INSERT INTO display_field_values (display_field_id, survey_response_id, value, created_at, updated_at)
        VALUES #{attribs.join(",")};
      SQL

      DisplayFieldValue.connection.execute(insert_sql);
    end
  end
end
