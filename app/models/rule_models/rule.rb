...
class Rule < ActiveRecord::Base
  ...
  validates :rule_order, :presence => true, :uniqueness => {
    :scope => :survey_version_id
  }
  ...
  validates :email_action, :presence => true,
    :if => Proc.new {|rule|
  ...
  accepts_nested_attributes_for :criteria,
    :reject_if => lambda {|attrs|
      !["9", "10"].include?(attrs[:conditional_id].to_s) && attrs[:value].blank?
    },
    :allow_destroy => true
  accepts_nested_attributes_for :actions,
    :reject_if => lambda {|attrs|
      attrs[:value].blank? || attrs[:action_type] == 'email'
    },
    :allow_destroy => true
  accepts_nested_attributes_for :email_action, :reject_if => lambda {|attrs|
      attrs[:action_type] == 'db' || attrs['body'].blank?
    },
    :allow_destroy => true
  accepts_nested_attributes_for :execution_trigger_rules,
    :reject_if => lambda {|attrs|
      attrs[:execution_trigger_id].blank?
    },
    :allow_destroy => true
  ...
  def increment_rule_order
    ...
    unless target_rule_order > self.survey_version.display_fields.count
      self.move_rule(target_rule_order)
    end
  end
  ...
  #
  # @param [Integer] target_rule_order the index of the Rule order to move
  # the Rules around.
  def move_rule(target_rule_order)
    Rule.transaction do
      start_index, end_index = target_rule_order < self.rule_order ?
        [target_rule_order,self.rule_order] : [self.rule_order,target_rule_order]

      # shift range from target to source (overwrite source with it's neighbor
      # in direction of target)
      if target_rule_order < self.rule_order
        self.survey_version.rules
          .where(['rule_order >= ? and rule_order < ?', start_index, end_index])
          .update_all("rule_order = rule_order + 1")
      else
        self.survey_version.rules
          .where(['rule_order > ? and rule_order <= ?', start_index, end_index])
          .update_all("rule_order = rule_order - 1")
  ...
  def clone_me(target_sv)
    ...
    a_attribs = self.actions.map do |action|
      ...
      new_display_field = DisplayField.find_by_survey_version_id_and_clone_of_id(
        target_sv.id,
        action.display_field_id
      )
      # If we can't find the newly cloned display field then this is now an
      # invalid action so don't clone the rule
      next unless new_display_field

      if action.value_type == "Response"
        response_qc = self.survey_version.options_for_action_select.select {|x|
          x[1] == action.value
        }.first
        next if response_qc.nil?

        new_value = target_sv.options_for_action_select.select {|x|
          x[0] == response_qc[0]
        }.try(:first).try(:last)
    ...
    c_attribs = self.criteria.map do |criterion|
      ...
      # if we can't find the new source clone then this is now an invalid
      # criterion so don't clone the rule
  ...
  def reorder_rules
    self.survey_version.rules
      .update_all(
        'rules.rule_order = rules.rule_order - 1',
        ['rules.rule_order > ?', self.rule_order]
      )
  end
end
