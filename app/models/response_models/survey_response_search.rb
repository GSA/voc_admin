...
class SurveyResponseSearch
  ...
  def build_condition_string
    @criteria.each do |k, criteria_fields|
      ...
      if %w(survey_responses.page_url survey_responses.created_at survey_responses.device survey_responses.referrer survey_responses.user_agent).include?(criteria_fields['display_field_id'])
