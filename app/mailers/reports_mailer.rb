...
class ReportsMailer < ActionMailer::Base
  ...
  def report_pdf(report_id, emails, from_user, report_frequency)
    ...
    url = pdf_report_url(@report.survey_version.survey_id, @report.survey_version_id, @report, :format => :pdf, :host => APP_CONFIG['admin_host'])
    file = open("#{url}?token=#{token}&salt=#{salt}", :allow_redirections => :safe).read
