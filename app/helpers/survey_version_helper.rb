module SurveyVersionHelper
  ...
  def page_management_links(survey, survey_version, page)
		...
		str += link_to "Copy Page", copy_page_survey_survey_version_page_path(survey, survey_version, page), :method => :post, :title=>"Create a copy of this page.", :remote => true, :class=>"copy_page"
		str += link_to image_tag("red_x.png", :alt=>"Remove Page", :class=>"redXpage"), survey_survey_version_page_path(survey, survey_version, page), :remote => true, :method => :delete, :confirm => "All items on page #{page.page_number} will be removed as well.", :class => "remove_page_link", :title => "Remove page"    
  end
